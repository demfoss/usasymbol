...
namespace USASymbol.Services
{
    public class FlowerService : IFlowerService
    {
        ...
        public FlowerService(IMemoryCache cache, IWebHostEnvironment env)
        {
            _cache = cache;
            _env = env;
        ...
        public async Task<FlowerContent?> GetFlowerContentAsync(string stateSlug)
        {
            var path = Path.Combine(_env.ContentRootPath, "Content", "states", stateSlug, "flower.yaml");

            if (!File.Exists(path))
                return null;

            var cacheKey = $"flower-{stateSlug}-{File.GetLastWriteTimeUtc(path).Ticks}";
            ...
            return await _cache.GetOrCreateAsync(cacheKey, async entry =>
            {
                ...
                try
                {
                    ...
                    var flowerContent = new FlowerContent
                    {
                    ...
                    {
                        flowerContent.SeoTitle = GetString(seo, "title");
                        flowerContent.SeoDescription = GetString(seo, "description");
                    }
                    else
                    {
                        flowerContent.SeoTitle = GetString(data, "seo_title");
                        flowerContent.SeoDescription = GetString(data, "seo_description");
                    }
                    ...
                    if (data.ContainsKey("sections") && data["sections"] is List<object> sections)
                    {
                        foreach (var sec in sections)
                        {
                            if (sec is Dictionary<object, object> secDict)
                            {
                                ...
                                {
                                    var flowerSubsections = new List<FlowerSubsection>();

                                    section.Subsections = flowerSubsections.Cast<IContentSubsection>().ToList();
                                    foreach (var sub in subsections)
                                    {
                                        if (sub is Dictionary<object, object> subDict)
                                        {
                                            var subsection = new FlowerSubsection
                                            {
                                ...
                                flowerContent.Sections.Add(section);
                            }
                        }
                    }
                    ...
                    if (data.ContainsKey("faq") && data["faq"] is List<object> faqList)
                    {
                        foreach (var faq in faqList)
                        {
                            ...
                            {
                                flowerContent.Faq.Add(new FlowerFaq
                                {
                    ...
                    if (data.ContainsKey("sources") && data["sources"] is List<object> sources)
                    {
                        foreach (var src in sources)
                        {
                            ...
                            {
                                flowerContent.Sources.Add(new FlowerSource
                                {
                    ...
                    {
                        flowerContent.BigStat = new BigStatData
                        {
                    ...
                    if (data.TryGetValue("timeline", out var timelineObj) && timelineObj is List<object> timelineList)
                    {
                        foreach (var item in timelineList.OfType<Dictionary<object, object>>())
                        {
                            ...
                            flowerContent.Timeline.Add(new TimelineEvent
                            {
                                Year = year,
                    ...
                    {
                        flowerContent.ExpertQuote = new ExpertQuoteData
                        {
                    ...
                    if (data.TryGetValue("quick_facts", out var quickFactsObj) && quickFactsObj is List<object> quickFactsList)
                    {
                        foreach (var fact in quickFactsList)
                        {
                            ...
                            flowerContent.QuickFacts.Add(new QuickFactItem
                            {
                                Label = label.Trim(),
                                Value = value.Trim(),
                    ...
                    if (flowerContent.QuickFacts.Count == 0)
                    {
                        flowerContent.QuickFacts = BuildQuickFacts(flowerContent);
                    }



                    flowerContent.VisualAssets = YamlParse.VisualAssets(data);

                    return flowerContent;
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error parsing flower.yaml for {stateSlug}: {ex.Message}");
        ...
        private static List<QuickFactItem> BuildQuickFacts(FlowerContent content)
        {
            ...
            AddFact("Status", content.IsOfficial ? "Official symbol" : "State flower");
