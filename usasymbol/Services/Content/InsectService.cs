...
namespace USASymbol.Services.Content
{
    public class InsectService : IInsectService
    {
        ...
        public InsectService(IMemoryCache cache, IWebHostEnvironment env)
        {
            _cache = cache;
            _env = env;
        ...
        public async Task<InsectContent?> GetInsectContentAsync(string stateSlug, string contentFileName = "insect.yaml")
        {
            var normalizedFileName = string.IsNullOrWhiteSpace(contentFileName) ? "insect.yaml" : contentFileName.Trim();
            var path = Path.Combine(_env.ContentRootPath, "Content", "states", stateSlug, normalizedFileName);

            if (!File.Exists(path))
                return null;

            var cacheKey = $"insect-content-{stateSlug}-{normalizedFileName}-{File.GetLastWriteTimeUtc(path).Ticks}";
                ...
                try
                {
                        ...
                        foreach (var sec in sections)
                        {
                            ...
                            if (secDict.ContainsKey("sites") && secDict["sites"] is List<object> siteList)
                            {
                                foreach (var site in siteList)
                                {
                                    ...
                                    section.Sites.Add(new InsectSite
                                    {
                    ...
                    if (data.ContainsKey("sources") && data["sources"] is List<object> sources)
                    {
                        foreach (var src in sources)
                        {
                            ...
                            {
                                content.Sources.Add(new InsectSource
                                {
        ...
        private static List<QuickFactItem> BuildQuickFacts(InsectContent content)
        {
            ...
            AddFact("Status", content.IsOfficial ? "Official state insect" : "State insect");
