...
{
    public class AmphibianService : IAmphibianService
    {
        private readonly IMemoryCache _cache;
        private readonly IWebHostEnvironment _env;
        private readonly IDeserializer _yamlDeserializer;

        public AmphibianService(IMemoryCache cache, IWebHostEnvironment env)
        {
            _cache = cache;
            _env = env;
        ...
        public async Task<AmphibianContent?> GetAmphibianContentAsync(string stateSlug, string contentFileName = "amphibian.yaml")
        {
            var normalizedFileName = string.IsNullOrWhiteSpace(contentFileName) ? "amphibian.yaml" : contentFileName.Trim();
            var path = Path.Combine(_env.ContentRootPath, "Content", "states", stateSlug, normalizedFileName);

            if (!File.Exists(path))
                return null;

            var cacheKey = $"amphibian-content-{stateSlug}-{normalizedFileName}-{File.GetLastWriteTimeUtc(path).Ticks}";
            ...
            return await _cache.GetOrCreateAsync(cacheKey, async entry =>
            {
                ...
                try
                {
                    ...
                    var content = new AmphibianContent
                    {
                    ...
                    if (data.ContainsKey("sections") && data["sections"] is List<object> sections)
                    {
                        foreach (var sec in sections)
                        {
                            ...
                            var section = new AmphibianSection
                            {
                            ...
                            if (secDict.ContainsKey("sites") && secDict["sites"] is List<object> siteList)
                            {
                                foreach (var site in siteList)
                                {
                                    ...
                                    section.Sites.Add(new AmphibianSite
                                    {
                    ...
                    if (data.ContainsKey("faq") && data["faq"] is List<object> faqList)
                    {
                        foreach (var faq in faqList)
                        {
                            ...
                            {
                                content.Faq.Add(new AmphibianFaq
                                {
                    ...
                    if (data.ContainsKey("sources") && data["sources"] is List<object> sources)
                    {
                        foreach (var src in sources)
                        {
                            ...
                            {
                                content.Sources.Add(new AmphibianSource
                                {
        ...
        private static List<QuickFactItem> BuildQuickFacts(AmphibianContent content)
        {
            ...
            AddFact("Scientific Name", content.BinomialName, italic: true);
            if (content.AdoptedYear.HasValue && content.AdoptedYear.Value > 0)
                AddFact("Adopted", content.AdoptedYear.Value.ToString());

            AddFact("Status", content.IsOfficial ? "Official state amphibian" : "State amphibian");
