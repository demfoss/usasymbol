...
namespace USASymbol.Services.Content
{
    public class DanceService : IDanceService
    {
        ...
        public DanceService(IMemoryCache cache, IWebHostEnvironment env)
        {
            _cache = cache;
            _env = env;
        ...
        public async Task<DanceContent?> GetDanceContentAsync(string stateSlug, string symbolSlug)
        {
            var path = Path.Combine(_env.ContentRootPath, "Content", "states", stateSlug, "dance", $"{symbolSlug}.yaml");

            if (!File.Exists(path))
                return null;

            var cacheKey = $"dance-content-{stateSlug}-{symbolSlug}-{File.GetLastWriteTimeUtc(path).Ticks}";
                ...
                try
                {
                    ...
                    if (data.ContainsKey("sections") && data["sections"] is List<object> sections)
                    {
                        foreach (var sec in sections)
                        {
                            ...
                            if (secDict.ContainsKey("steps") && secDict["steps"] is List<object> steps)
                            {
                                section.Steps = new List<DanceStep>();
                                foreach (var step in steps)
                                {
                                    if (step is not Dictionary<object, object> stepDict)
                                        continue;

                                    section.Steps.Add(new DanceStep
                                    {
                                        Title = GetString(stepDict, "title"),
                                        Description = GetString(stepDict, "description")
                                    });
                                }
                            }
                    ...
                    if (data.ContainsKey("sources") && data["sources"] is List<object> sources)
                    {
                        foreach (var src in sources)
                        {
                            ...
                            content.Sources.Add(new DanceSource
                            {
        ...
        private static List<QuickFactItem> BuildQuickFacts(DanceContent content)
        {
            ...
            AddFact("Status", content.IsOfficial ? "Official state dance" : "State dance");
