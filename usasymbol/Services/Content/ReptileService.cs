...
namespace USASymbol.Services.Content
{
    public class ReptileService : IReptileService
    {
        ...
        public ReptileService(IMemoryCache cache, IWebHostEnvironment env)
        {
            _cache = cache;
            _env = env;
        ...
        public async Task<ReptileContent?> GetReptileContentAsync(string stateSlug, string contentFileName = "reptile.yaml")
        {
            var normalizedFileName = string.IsNullOrWhiteSpace(contentFileName) ? "reptile.yaml" : contentFileName.Trim();
            var path = Path.Combine(_env.ContentRootPath, "Content", "states", stateSlug, normalizedFileName);

            if (!File.Exists(path))
                return null;

            var cacheKey = $"reptile-content-{stateSlug}-{normalizedFileName}-{File.GetLastWriteTimeUtc(path).Ticks}";
            ...
            return await _cache.GetOrCreateAsync(cacheKey, async entry =>
            {
                ...
                try
                {
                    ...
                    var content = new ReptileContent
                    {
                    ...
                    if (data.ContainsKey("sections") && data["sections"] is List<object> sections)
                    {
                        foreach (var sec in sections)
                        {
                            ...
                            var section = new ReptileSection
                            {
                            ...
                            if (secDict.ContainsKey("sites") && secDict["sites"] is List<object> siteList)
                            {
                                foreach (var site in siteList)
                                {
                                    ...
                                    section.Sites.Add(new ReptileSite
                                    {
                    ...
                    if (data.ContainsKey("faq") && data["faq"] is List<object> faqList)
                    {
                        foreach (var faq in faqList)
                        {
                            ...
                            {
                                content.Faq.Add(new ReptileFaq
                                {
                    ...
                    if (data.ContainsKey("sources") && data["sources"] is List<object> sources)
                    {
                        foreach (var src in sources)
                        {
                            ...
                            {
                                content.Sources.Add(new ReptileSource
                                {
        ...
        private static List<QuickFactItem> BuildQuickFacts(ReptileContent content)
        {
            ...
            AddFact("Status", content.IsOfficial ? "Official state reptile" : "State reptile");
