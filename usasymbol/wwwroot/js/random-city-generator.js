(function () {
    ...
    var panel = document.getElementById("city-generator");
    if (!panel) return;

    var data = window.cityGeneratorData || {};
    var generateUrl = panel.getAttribute("data-generate-url");

    var countInput = document.getElementById("cg-count");
    var stateSelect = document.getElementById("cg-state");
    var generateBtn = document.getElementById("cg-generate");
    var downloadBtn = document.getElementById("cg-download");
    ...
    function cardMarkup(entry) {
        var lineText = entry.city + ", " + entry.stateCode;
        return (
            '<div class="zg-card">' +
    ...
    function render(results) {
        ...
        if (!currentResults.length) {
            grid.innerHTML = '<div class="zg-empty">No cities found. Try a different state.</div>';
            return;
        }
    ...
    function generate() {
        ...
        fetch(url, { headers: { "Accept": "application/json" } })
            ...
            .then(function (results) {
                render(results);
                setStatus(results.length + " cit" + (results.length === 1 ? "y" : "ies") + " generated.", false);
            })
            .catch(function () {
                setStatus("Something went wrong generating cities. Please try again.", true);
            })
            .finally(function () {
    ...
    function downloadTxt() {
        ...
        var lines = currentResults.map(function (entry) {
            return entry.city + ", " + entry.stateCode;
        });
